...
static UA_StatusCode
readMyCounter(UA_Server *server,
              const UA_NodeId *sessionId, void *sessionContext,
              const UA_NodeId *nodeId, void *nodeContext,
              UA_Boolean sourceTimeStamp, const UA_NumericRange *range,
              UA_DataValue *dataValue) {

    UA_Int32* myCounter = nodeContext;
    UA_Variant_setScalarCopy(&dataValue->value, myCounter,
...
static void
addCounterVariable(UA_Server* server) {
    UA_VariableAttributes attr = UA_VariableAttributes_default;
    attr.description = UA_LOCALIZEDTEXT("en-US","myCounter");
    attr.displayName = UA_LOCALIZEDTEXT("en-US","myCounter");
    attr.dataType = UA_TYPES[UA_TYPES_INT32].typeId;
    attr.accessLevel = UA_ACCESSLEVELMASK_READ;

    /* Add the variable node to the information model */
    UA_NodeId myCounterNodeId = UA_NODEID_STRING(1, "myCounter");
    UA_QualifiedName myCounterName = UA_QUALIFIEDNAME(1, "myCounter");
    UA_NodeId parentNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
    UA_NodeId parentReferenceNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES);
    UA_NodeId variableTypeNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE);

    UA_DataSource myCounterDataSource;
    myCounterDataSource.read = readMyCounter;
    myCounterDataSource.write = NULL;

    UA_Int32* myCounter = malloc(sizeof *myCounter);
    *myCounter = 0;

    UA_Server_addDataSourceVariableNode(server, myCounterNodeId, parentNodeId,
                                        parentReferenceNodeId, myCounterName,
                                        variableTypeNodeId, attr,
                                        myCounterDataSource,
                                        myCounter, NULL);
